...
function checksExistsUserAccount(request, response, next) {
  const isValidHeader = !!request?.headers?.username;
  if (!isValidHeader) {
...
function checksCreateTodosUserAvailability(request, response, next) {
  ...
  if (!user) {
    return response
      .status(403)
      .json({ error: "You limit of todos is exceded." });
  }
...
function checksTodoExists(request, response, next) {
  ...
  if (!user) {
    return response.status(404).json({ error: "User not fuund" });
  }
  ...
  if (!toDo) {
    return response
      .status(404)
      .json({ error: "Todo not found, in todos of user." });
  }
